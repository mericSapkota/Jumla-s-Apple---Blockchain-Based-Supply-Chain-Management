...
export default function StatusBadge({ status, className = "" }) {
  const info = STATUS_MAP[status] || { emoji: "•", label: status, className: "bg-surface-container text-on-surface" };
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase text-on-surface tracking-wider ${info.className} ${className}`}
    >
